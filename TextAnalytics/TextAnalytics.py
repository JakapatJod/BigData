...
from pyspark.sql import functions as F  # ใช้ฟังก์ชัน SQL ทั่วไป (ไม่ได้ใช้ในสคริปต์นี้)
...
spark = SparkSession.builder \
    ...
    .getOrCreate()  # สร้าง session ใหม่หรือใช้ session ที่มีอยู่แล้ว
...
# Pipeline
pipeline = Pipeline(stages=[tokenizer, stop_word_remover, hashing_tf])  # กำหนดลำดับขั้นตอนการประมวลผล (tokenizer, stop words remover, และ hashing TF)
...
train_df.show(truncate=False)  # แสดงข้อมูลชุดฝึกที่ผ่านการแปลงแล้ว (มีคำที่ถูกแยก คำที่มีความหมาย และฟีเจอร์)

# Logistic Regression
lr = LogisticRegression(labelCol="Rating", featuresCol="features")  # สร้างโมเดล Logistic Regression โดยใช้ "Rating" เป็น label และ "features" เป็นฟีเจอร์
...
predictions = lr_model.transform(test_df)  # ทำการทำนายชุดทดสอบด้วยโมเดล Logistic Regression ที่ฝึกแล้ว
